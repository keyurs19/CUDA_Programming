...
main(int argc, char** argv) {
	...
	gettimeofday(&stop, NULL);
		printf("Execution time using CPU = %fs. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));	
...
vec_mat_mult_on_device_using_global_memory(const Matrix A, const Matrix X, Matrix Y)
{
	...
	dim3 grid(1,MATRIX_SIZE/TILE_SIZE);
	struct timeval start, stop;	
		gettimeofday(&start, NULL);
	/* Launch kernel. */
	...
	cudaThreadSynchronize();
    	...
    	gettimeofday(&stop, NULL);
		printf("Execution time using global memory = %fs. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));	
...
vec_mat_mult_on_device_using_shared_memory(const Matrix A, const Matrix X, Matrix Y)
{
	...
	dim3 grid(TILE_SIZE,MATRIX_SIZE/TILE_SIZE);
	struct timeval start, stop;	
		gettimeofday(&start, NULL);
	/* Launch kernel. */
